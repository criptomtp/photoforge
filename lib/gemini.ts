...
ГОЛОВНИЙ ПРИНЦИП (найважливіше): референс — це джерело інформації ТІЛЬКИ про товар-герой (його форму, крій, посадку, колір, текстуру, матеріал, шви, фурнітуру, принти, логотипи). Усе інше на референсі — модель, її обличчя, поза, випадкові інші предмети одягу (сорочка, футболка під низом, куртка, аксесуари), фон, реквізит — НЕ переноситься. Ти будуєш НОВИЙ кадр з НОВОЮ моделлю. Заборонено репродукувати референс-фото цілком.

КАТЕГОРІЯ ТОВАРУ: {CATEGORY}. {SUBJECT}

ОБОВ'ЯЗКОВІ ПРАВИЛА:

1) ТОВАР-ГЕРОЙ — ЦЕ ЛИШЕ ЦІЛЬОВИЙ ТОВАР, А НЕ ВЕСЬ РЕФЕРЕНС
— З референсу бери ТІЛЬКИ сам цільовий товар (наприклад джинси): його форму, крій, посадку, колір і відтінок, текстуру, матеріал, шви, фурнітуру, принти, логотипи й написи.
— КАТЕГОРИЧНО НЕ переноси з референсу інші предмети одягу та аксесуари, які випадково одягнені на референс-моделі (сорочку, футболку/майку під низом, светр, куртку, шапку, взуття, годинник, сумку, прикраси). Багатошаровість одягу з референсу ІГНОРУЙ. Решту образу збери ЗАНОВО — чисто, мінімалістично й доречно до сезону, щоб товар-герой лишався головним акцентом.
...
ГОЛОВНИЙ ПРИНЦИП: референс — це джерело інформації ТІЛЬКИ про товар-герой (форма, колір, текстура, матеріал, написи, логотипи, пропорції). Усе інше на референсі — фон, реквізит, чужі предмети, руки, освітлення — НЕ переноситься. Ти будуєш НОВИЙ кадр. Заборонено репродукувати референс-фото цілком.

КАТЕГОРІЯ ТОВАРУ: {CATEGORY}. {SUBJECT}

ОБОВ'ЯЗКОВІ ПРАВИЛА:

1) ТОВАР-ГЕРОЙ — ЦЕ ЛИШЕ ЦІЛЬОВИЙ ТОВАР
— З референсу бери ТІЛЬКИ сам товар: форму, колір і відтінок, текстуру, матеріал, написи, логотипи, пропорції.
— НЕ переноси з референсу сторонні предмети, реквізит, руки, фон. Якщо на референсі товар серед інших речей — ізолюй сам товар.
...
function fillTemplate(tpl: string, N: number, cat: Category, angleList: string): string {
  return tpl
...
function buildSystemPrompt(angles: AngleDef[], cat: Category): string {
  const N = angles.length;
  const angleList = angles.map((a, i) => `${i + 1}. ${a.label} — ${a.desc}`).join("\n");
  return fillTemplate(cat.onModel ? ONMODEL_SYSTEM : OBJECT_SYSTEM, N, cat, angleList);
}
...
const ONMODEL_IMAGE_INSTRUCTIONS = `ЗАВДАННЯ: згенеруй НОВЕ професійне фото товару — свіжий каталоговий кадр на новій моделі. Це НЕ редагування й НЕ відтворення доданого фото. Додане фото — лише візуальний довідник по товару-герою, а не шаблон, який треба скопіювати.

ЩО БЕРЕТЬСЯ З РЕФЕРЕНСУ — ТІЛЬКИ ТОВАР-ГЕРОЙ:
— З доданого фото візьми ВИКЛЮЧНО сам товар-герой (той, що описаний у промпті вище — наприклад джинси): його форму, крій, посадку, колір і відтінок, текстуру, матеріал, шви, фурнітуру, принти, логотипи й написи. Відтвори товар достовірно й охайно. Логотипи та текст на товарі — як на референсі, не перемальовуй і не вигадуй; деталь не видно — не домальовуй.
...
— Решту образу (одяг, окрім товару-героя) стилізуй заново — чисто, мінімалістично, доречно до сезону, щоб товар лишався головним акцентом. НЕ копіюй випадковий одяг із референсу.
...
const OBJECT_IMAGE_INSTRUCTIONS = `ЗАВДАННЯ: згенеруй НОВЕ професійне продуктове фото товару. Це НЕ редагування й НЕ відтворення доданого фото. Додане фото — лише довідник по товару-герою, а не шаблон для копіювання.

ЩО БЕРЕТЬСЯ З РЕФЕРЕНСУ — ТІЛЬКИ ТОВАР-ГЕРОЙ:
— Візьми ВИКЛЮЧНО сам товар (описаний у промпті вище): форму, колір і відтінок, текстуру, матеріал, написи, логотипи, пропорції. Відтвори товар достовірно й охайно. Написи/лого — як на референсі; деталь не видно — не домальовуй.
— Згадка про точність товару тут одна. НЕ сприймай це як команду «скопіюй усе фото».
...
export function imageInstructionsFor(cat: Category): string {
  return cat.onModel ? ONMODEL_IMAGE_INSTRUCTIONS : OBJECT_IMAGE_INSTRUCTIONS;
}
...
export async function generatePrompts(
  ...
  const body = {
    systemInstruction: {
      parts: [{ text: buildSystemPrompt(angles, cat) }],
    },
    contents: [
      {
        role: "user",
...
export async function generateImage(
  ...
  instructions: string = ONMODEL_IMAGE_INSTRUCTIONS
): Promise<string> {
